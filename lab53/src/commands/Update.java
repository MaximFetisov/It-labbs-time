...
public boolean execute(String[] args) {
    ...
    for (int i = 0; i < Run.collectionManager.getCities().size(); i++) {
        id_map.put(Run.collectionManager.getCities().get(i).getId(), i);
    }
    ...
    try {
        ...
        if (id_map.containsKey(id)) {
            Run.collectionManager.getCities().set((int) id_map.get(id), new City(id));
            Run.inout.write(Run.collectionManager.getCities().get((int) id_map.get(id)).toString());
        } else {
