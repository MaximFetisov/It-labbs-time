...
public class CityForm {
    ...
    while (true) {
        ...
        String normalized = test.trim().replace(",", ".");
        if (normalized.contains(".") && normalized.split("\\.")[1].length() > 2) {
            Run.inout.write("Слишком много знаков после запятой (макс. 2). Попробуйте снова.");
            continue;
        }
        ...
        try {
            ...
            Run.inout.write("Неверный формат числа. Попробуйте снова.");
        }
    }
}
